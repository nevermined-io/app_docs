...
after you bought a payment plan.
...
that allows to query it. The steps to integrate are the following:

### 1. Open your Dashboard page

Click on the "My Assets" link on the header of the application. Select the "My Assets" option in the
...
You can access detailed information about the agent clicking the **eye icons** on the right hand
side.
...
Go to the **"Endpoints"** tab and select the endpoint of the list you want to test. Then click on
...
At this point the Nevermined App will send a HTTP request to the upstream agent via the Proxy
...
First you need to compose the **Proxy URL**. The Proxy URL is the full URL you are gonna call to
...
Nevermined provides the [Payments Libraries](../../libraries/) which allow you to integrate the
agent in your application. The libraries are available in different languages and provide a simple
way to interact with the Nevermined ecosystem and another agents.
...
:::info If you want to learn more about how order payment plans, please check the
[How Subscribers can order Plans?](../../libraries/order-plans) section of the documentation. :::
