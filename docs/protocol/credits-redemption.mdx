...
2. The user via Nevermined libraries get the JWT access token to the Agent via the Nevermined Node
   (this process only needs to be do once per agent, not per request)
3. The user sends a HTTP request to the AI Agent through a Nevermined Proxy. The user sends in the
...
9. Asynchronously the Proxy validates the number of costs in credits reported by the AI Agent and if
   it’s between the minimum and maximum, redeems (burn) these credits.

<br />

The AI Agent only can report credits to redeem between the thresholds specified in the AI Agent
