...
# Welcome to Nevermined App

At Nevermined, we see a future in which the only limitations to AI advancement will be their ability to access and transact on the data and services required by the agent to optimize their inferences. There are 2 issues at present that will impede this progress:

1. AI Agents won’t be able to get a bank account (they will have wallets on-chain instead).
1. Web3 UX for humans and AIs alike still SUCKS.
...
## NFT Subscriptions → Problem Solved

So, we went back to the drawing board to simplify the process and created Subscription NFTs. Nevermined’s Subscription NFTs address the two issues above as follows:
...
2. NFT subscriptions allow data and services Publishers to create one NFT, append registered data and service assets to the NFT, and then set the tokengating parameters, like how much time access is granted for how much, that enforce the Subscription conditions.
